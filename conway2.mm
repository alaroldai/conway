...
class Conway {
	...
	int staticUpdate() {
		int threadMax = 4;
		vector<boost::thread *> threads(threadMax);
		for (int i = 0; i < threadMax; i++) {
			CGRect bounds = CGRectMake(0, i * (resolution.height / threadMax), resolution.width, resolution.height / threadMax);
